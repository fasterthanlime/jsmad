...
Mad.AjaxStream.prototype.getb = function(offset, length) {
    if (this.absoluteAvailable(offset + length)) {
		if(this.state.byteBuffer) {
			return this.state.byteBuffer.subarray(offset, offset + length);
		} else {
			var buffer = new Uint8Array(length);
			for(var j = 0, i = offset; i < offset + length; i += 1, j += 1) {
				buffer[j] = this.state.buffer.charCodeAt(i) & 0xff;
			}
			return buffer;
		}
    } else {
